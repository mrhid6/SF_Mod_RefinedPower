...
void ARPSolarPanel::BeginPlay()
{
    ...
    if (HasAuthority())
    {
        ...
        GetWorld()->GetTimerManager().SetTimer(mSolarPanelHandle, this, &ARPSolarPanel::UpdateLineTraceRotation, 5.0f,
                                               true);
    }
...
void ARPSolarPanel::SpawnPanel()
{
    ...
    if (mSolarController && mRotatesTowardSun && hasSpawnedPanel == false)
    {
        ...
        UE_LOG(LogRP, Verbose, TEXT("[RP] - Spawned SolarPanel Panel!"));
    }else
    {
        /*SML::Logging::info("########################");
...
ARPSolarController* ARPSolarPanel::GetSolarController()
{
    if (mSolarController == nullptr)
    {
        ...
        if(mSolarController == nullptr)
        {
            UE_LOG(LogRP, Verbose, TEXT("[RP] - Still can't get controller!"))
        }else
        {
            UE_LOG(LogRP, Verbose, TEXT("[RP] - Got controller!"))
        }
    }
