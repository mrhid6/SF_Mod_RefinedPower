...
void ARPMPBuilding::BeginPlay()
{
    ...
    if (HasAuthority())
    {
        ...
        if (!MPInventoryComponent)
        {
            MPInventoryComponent = UFGInventoryLibrary::CreateInventoryComponent(this, FName("MPInventory" + GetName()));
            if (MPInventoryComponent->GetSizeLinear() != 2)
            {
                MPInventoryComponent->Resize(2);
            }
            UE_LOG(LogRP, Display, TEXT("Created Inventory: %s"), *MPInventoryComponent->GetName());
        }
        else
        {
            if (MPInventoryComponent->GetSizeLinear() != 2)
            {
                MPInventoryComponent->Resize(2);
                UE_LOG(LogRP, Display, TEXT("Inventory Loaded needing Resze: %s"), *MPInventoryComponent->GetName());

            }
            UE_LOG(LogRP, Display, TEXT("Inventory Loaded: %s"), *MPInventoryComponent->GetName());
...
bool ARPMPBuilding::ShouldSave_Implementation() const
{
    return true;
}
...
void ARPMPBuilding::GetDismantleRefund_Implementation(TArray<FInventoryStack>& out_refund) const
{
    ...
    // Null check for machines that don't have an inventory (generators)
    if (GetMPInventory() == nullptr) return;
...
void ARPMPBuilding::CalculateUsageRates()
{
    ...
    UE_LOG(LogRP, Display, TEXT("#####################"));
    UE_LOG(LogRP, Display, TEXT("Consumption:"));
    UE_LOG(LogRP, Display, TEXT("%f"), mConsumptionTotal);
    UE_LOG(LogRP, Display, TEXT("Production:"));
    UE_LOG(LogRP, Display, TEXT("%f"), mProductionTotal);
    UE_LOG(LogRP, Display, TEXT("#####################"));
